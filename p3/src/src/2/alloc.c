...
{
//algoritmo cambiado a best fit (parte modificada)
	...
	hp = hole_head;
         ...
         if (candidate != NIL_HOLE) {
               hp = candidate; prev_ptr = last_hole_prev;
      			/* se encontro un hole que es lo suficientemente grande, usarlo! */
      			old_base = hp->h_base;	/* recuerda donde empezo  */
      			hp->h_base += clicks;	
      			hp->h_len -= clicks;	
      
      			/* recuerda el nuevo high watermark de memoria usada */
      			if(hp->h_base > high_watermark)
      				high_watermark = hp->h_base;
      
      			/*borrar el hole si se uso completamente */
      			if (hp->h_len == 0) del_slot(prev_ptr, hp);
      
      			/* volver al principio de la direccion de memoria del bloque adquirido */
      			return(old_base);
         }

     
   
  } while (swap_out());		/* try to swap some other process out */ //esto es del source code inicial 
